...
function RecipeDetails({ allRecipes }) {
    ...
    function splitInstructions(recipeInstructions) {
        return recipeInstructions.match(/[^\.!\?]+[\.!\?]+/g).map((sentence, index) => {
            return <li className="text-justify md:text-lg p-1" key={index}>{sentence}</li>
        })
    }
        ...
        <>
            <div className="">
                ...
                <div className="h-full flex flex-grid grid-cols-2 md:grid-cols-3 justify-center md:justify-center px-6">
                    {allRecipes.map(recipe => recipe._id === id ?
                        <div className="flex flex-col items-center py-6 md:py-10 px-2 mb-4" key={recipe._id}>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 lg:gap-16">
                                ...
                                <div className="flex flex-col items-center justify-center">
                                    <h1 className="text-center sm:text-xl md:text-2xl font-semibold pb-2 sm:py-4">{capitaliseRecipeName(recipe.recipeName)}</h1>
                                    ...
                                    <div className="flex flex-row gap-2 sm:gap-4 py-2 sm:py-4">
                                        ...
                                        <button className="border border-black md:text-xl py-1 px-2 sm:py-1.5 sm:px-4 rounded-lg">Share</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="w-full max-w-5xl flex flex-col sm:flex-row justify-between gap-6 sm:gap-0 py-2 md:py-4 lg:py-10">
                                <div className="w-full sm:w-1/3 flex flex-col sm:mr-4">
                                    <h2 className="text-lg md:text-xl font-semibold border-b py-1 my-4">Ingredients</h2>
                                    ...
                                    <button className="bg-mandarin text-white rounded-md py-1.5 mt-10">Email me my shopping list</button>
                                </div>
                                <div className="w-full sm:w-2/3 sm:ml-20">
                                    <h2 className="text-lg md:text-xl font-semibold border-b py-1 my-4">Instructions</h2>
